...
for i=1:Rows_A
    ...
    for v=1:mu
        ...
        if index_state(i,v) ~= 0
           ...
           elseif MAK == 1 && network == 0
               ...
               inputs = num2cell([New(i, Columns), Y]);
               A(i,index_state(i,v))=W_star{v}(inputs{:});
           end
    ...
    elseif MAK == 1 && network == 0
        ...
        for k=1:length(W_star)
         % General case for non-mass action kinetics
         inputs = num2cell([Hidden_Species(i, :), Y]); % Combine Hidden_Species and Y into a single input array
         W(k) = W_star{k}(inputs{:});
        end
    end
    A(i,i)=-sum(W);
