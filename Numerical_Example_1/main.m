function main()
    ...
    if ~isempty(project_root)
        ...
        % Display the contents of the markdown file
        display_project_structure(output_file);
    else
...
function project_root = find_project_root(starting_dir, root_marker)
    ...
    while true
        dir_content = dir(current_dir);
        dir_names = {dir_content.name};
        
        if any(strcmp(dir_names, root_marker))
...
function display_project_structure(file_path)
    fid = fopen(file_path, 'r');
    if fid == -1
        error('Cannot open the markdown file.');
    end
    
    % Read and display the file contents line by line
    disp('Contents of the project structure file:');
    while ~feof(fid)
        line = fgetl(fid);
        disp(line);
    end
