...
# Use relative imports to import modules from the sibling directory
from ..CRN import CRN
from .HiddenSubsystems import HiddenSubsystems
from .SparseMatrixStructure import SparseMatrixStructure
from .DistributionOfSubsystems import DistributionOfSubsystems
...
class CRNForContinuousTimeFiltering(CRN):
    ...
    def __init__(self, stoichiometric_matrix, species_names, parameters_names, reaction_names, propensities, \
        ...
        self.stoichiometric_matrix_df = pd.DataFrame(self.stoichiometric_matrix, index=self.species_names,
        ...
        if sorted(range_of_species.index.tolist()) != sorted(self.hidden_species):
            print(
                "Error in definition the filtering class: range_of_species is not compatible with the unknown species")
        else:
        ...
        if sorted(range_of_parameters.index.tolist()) != sorted(self.unkown_parameters):
            print(
                "Error in definition the filtering class: range_of_parameters is not compatible with the unknown species")
        else:
        ...
        if sorted(discretization_size_parameters.index.tolist()) != sorted(self.unkown_parameters):
            print(
                "Error in definition the filtering class: discretization_size_parameters is not compatible with the unknown species")
        else:
    ...
    def create_the_graph_for_decomposition(self):
        ...
        for reaction_j in self.reaction_names:
            ...
            parameters_species_affects_propensity_j = \
                set(self.propensities_dependencies[self.reaction_ordering[reaction_j]]).intersection(
                    self.unkown_parameters + self.hidden_species)
            # compute the hidden speceis whose state is changed by reaction j
            hidden_species_affected_by_reaction_j = \
                [species for species in self.hidden_species if
                 self.stoichiometric_matrix_df.loc[species, reaction_j] != 0]
        ...
        for i in range(len(clustering)):
            nodes_in_ith_clustering = [graph_for_decomposing_hidden_system.vs['names'][index] for index in
                                       clustering[i]]
    ...
    def identify_subsystems(self):
        ...
        for i in np.arange(1, self.get_number_of_subsystem() + 1, 1):
            ...
            subsystems.append(
                HiddenSubsystems(parameters, species, unobservable_reactions, xi_list, observable_reactions))
    ...
    def get_size_of_subsystems(self):
        ...
        for i in range(1, self.get_number_of_subsystem() + 1):
            ...
            df = self.discretization_size_parameters.loc[
                self.subsystems[i].parameters]  # the parameters relivant to this subystem
            if df.empty:
    ...
    def check_state_in_the_range(self, state, ordering):
        for name, count in ordering.items():
            if name in self.parameters_names:
                if state[count] > self.range_of_parameters.loc[name, 'max'] or state[count] < \
                        self.range_of_parameters.loc[name, 'min']:
                    return False
            else:
                if state[count] > self.range_of_species.loc[name, 'max'] or state[count] < self.range_of_species.loc[
                    name, 'min']:
                    return False
        return True
    ...
    def set_the_states_for_each_subsystems(self, index_subsystem):
        ...
        coords_parameters \
            = [np.linspace(self.range_of_parameters.loc[parameters, 'min'],
                           self.range_of_parameters.loc[parameters, 'max'],
    ...
    def filteringFFSP(self, Y_trajectory, Y_ordering, Time_Y, Initial_Distributions, tqdm_disable=False):
        ...
        if len(Y_trajectory[0]) != self.get_number_of_observable_species():
            print(
                "Error in filtering: the dimension of Y_trajectory does not compatible with the number of observable species.")
            return "Error in filtering"
        ...
        for i in range(1, self.get_number_of_subsystem() + 1):
            if len(Initial_Distributions[i]) != len(self.subsystems[i].states):
                print("Error in filtering: the length of Initial_Distribution[" + str(
                    i) + "] does not fit the size of subsystems")
                return "Error in filtering"
        ...
        for i in range(1, self.get_number_of_subsystem() + 1):
            output.append(
                DistributionOfSubsystems(self.subsystems[i].states, self.subsystems[i].parameter_species_ordering))
            output[i].extend_distributions([Time_Y[0]], [Initial_Distributions[i]])

        # Solve the filtering equation iteratively
        for t_index in tqdm(range(len(Time_Y) - 1), desc="Progress", unit=" Observable_Jumps", position=0,
                            disable=tqdm_disable):  # range(len(Time_Y)-1):
            for i in range(1, self.get_number_of_subsystem() + 1):
                ...
                if A.shape[0] >= 45:
                    time_list, unnormalized_distribution_list = \
                        MatrixExponentialKrylov.exp_AT_x(A, Time_Y[t_index], Time_Y[t_index + 1],
                                                         output[i].distribution_list[-1])
                else:
                    time_list = [Time_Y[t_index + 1]]
                    A = A.toarray()
                    unnormalized_distribution = scipy.linalg.expm(A * (Time_Y[t_index + 1] - Time_Y[t_index])).dot(
                        output[i].distribution_list[-1])
                ...
                if corresponding_xi in self.subsystems[i].xi_list:
                    ...
                    A = self.subsystems[i].A_jump_structure[index_xi].build_final_matrix(Y_trajectory[t_index],
                                                                                         Y_ordering)
    ...
    def filteringFFSP_return_final_distribution(self, Y_trajectory, Y_ordering, Time_Y, Initial_Distributions,
                                                tqdm_disable=False):
    ...
    def plot_unknown_parameter_distribution_final_time(self, distribution_list_from_filtering,
                                                       parameters_real_value=None):
        """
        ...
        fig, axs = plt.subplots(math.ceil(self.get_number_of_unknown_parameters() / 2), 2,
